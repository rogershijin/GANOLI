...
#             discr_opt.zero_grad()
#             self.manual_backward(total_discr_loss, retain_graph=True)
#             discr_opt.step()

#         for gen_opt in gen_opts:
#             gen_opt.zero_grad()
#             self.manual_backward(total_gen_loss + total_recon_loss, retain_graph=True)
#             gen_opt.step()
        generator_rna2atac_opt, generator_atac2rna_opt, discriminator_rna_opt, discriminator_atac_opt = self.optimizers()
    
        discriminator_rna_opt.zero_grad()
        discriminator_atac_opt.zero_grad()
        self.manual_backward(total_discr_loss)
        discriminator_rna_opt.step()
        discriminator_atac_opt.step()
        
        generator_rna2atac_opt.zero_grad()
        generator_atac2rna_opt.zero_grad()
        self.manual_backward(total_gen_loss)
        self.manual_backward(total_recon_loss)
        generator_rna2atac_opt.step()
        generator_atac2rna.step()
